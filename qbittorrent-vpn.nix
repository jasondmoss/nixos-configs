...
#
# The private key lives at /etc/wireguard/qbit.conf (root:root 0600) and is
# NEVER placed in the nix store or git. Provision it once with:
...
let
    ...
    # The session vars ($WAYLAND_DISPLAY etc.) are expanded by the caller's
    # shell and baked into argv, so they survive the sudo -> runuser environment
    # reset. The Wayland/X sockets live under XDG_RUNTIME_DIR — a filesystem
    # path that a netns does not isolate — so the GUI connects to the compositor
    # normally.
...
in
{
    # Resolver used *inside* the namespace. `ip netns exec` bind-mounts this
    # over /etc/resolv.conf; the system CoreDNS at 127.0.0.1 is not present in
    # the ns.
...
# Create the wg interface in the ROOT namespace first so its encrypted UDP
# socket egresses via the physical NIC, THEN move the interface into the
# namespace. The transport socket stays in the namespace where the interface was
# created — this is what lets the tunnel reach its endpoint while everything
# else inside "qbit" is fully isolated.
ip link add ${wgIface} type wireguard
ip link set ${wgIface} netns ${ns}

# Apply the Proton config (private key, peer, endpoint). `wg-quick strip` drops
...
${pkgs.iproute2}/bin/ip netns del ${ns} 2>/dev/null || true
    ...
    # into qBittorrent. Runs *inside* the qbit namespace, so it talks to the
    # tunnel gateway and to qBittorrent's Web UI on the namespace's own
    # loopback. Proton expires the mapping after ~60s, hence the 45s refresh
    # loop. The current port is also written to /run/qbit-forwarded-port for
    # visibility (`cat` it anytime).
...
last_port=0
while true; do
    # Refresh TCP + UDP leases; "1 0" lets the gateway choose the public port
    # (Proton returns the same one for both protocols).
    ...
    if [ -n "$port" ]; then
        ...
        if [ "$port" != "$last_port" ]; then
            ...
            # Requires Web UI enabled with "Bypass authentication for localhost"
            # so 127.0.0.1 needs no login. Harmless no-op when qBittorrent is
            # not running.
