# packages/pcp/package.nix
#
# Performance Co-Pilot package derivation.
#
# Adapted from the PCP project's own Nix packaging (upstream build/nix/package.nix)
# for this flake-less repo: the signature stays `{ pkgs, src }` and `src` is
# supplied by ./default.nix via fetchFromGitHub instead of a flake `self`.
#
# Version is parsed from VERSION.pcp (configure's source of truth), so a version
# bump only needs the rev/hash changed in ./default.nix.
#
{ pkgs, src, withGui ? true }:
let
  lib = pkgs.lib;
  ...
  # don't perturb the derivation hash.
  #
    ...
    filter = path: type:
      let
        ...
        isExcluded =
          ...
          (parentDir == srcPath + "/build" && baseName == "nix") ||
...
pkgs.stdenv.mkDerivation rec {
  ...
  nativeBuildInputs = with pkgs; [
    ...
    # qmake gives configure the `qmake` binary; dontUseQmakeConfigure below keeps
    # its setup-hook from hijacking PCP's autotools configurePhase.
    libsForQt5.qmake
    libsForQt5.qttools      # lrelease for .ts translation files
  ...
  # Keep the qmake setup-hook from replacing PCP's autotools configurePhase;
  # we only want qmake on PATH for the pmchart subdir build.
  dontUseQmakeConfigure = withQt;
  ...
  configureFlags = lib.concatLists [
    ...
    (if withQt then [ "--with-qt=yes" ] else [ "--with-qt=no" ])
  ...
  # PCP_DIR   : libpcp finds pcp.conf via $PCP_DIR/etc/pcp.conf on NixOS.
